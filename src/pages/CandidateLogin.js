...
const CandidateLogin = ({ quizId, onLoginSuccess }) => {
  ...
  const handleLogin = async (e) => {
    ...
    if (!isQuizActive) {
      setError(
        "The quiz is not currently active. Please contact the administrator."
      );
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (candidateFound) {
        ...
        await updateDoc(candidateDocRef, { status: "attending" });
      ...
      <div className='login-container'>
        <div className='login-card p-4' style={{ maxWidth: "600px" }}>
          ...
          <div className='candidate-details my-4 p-3 bg-light rounded'>
            <p>
              <strong>Place:</strong> {candidateData.place}
            </p>
            <p>
              <strong>Phone Number:</strong> {candidateData.phone}
            </p>
          </div>
          <hr />
          ...
          <ul className='guidelines'>
            <li>
              The quiz duration is <strong>{quiz.duration} minutes</strong>.
            </li>
            <li>
              Do not switch tabs or windows. Doing so twice will automatically
              submit your quiz.
            </li>
            <li>Copying, pasting, and right-clicking are disabled.</li>
            <li>Ensure you have a stable internet connection.</li>
            <li>
              Click "Start Quiz" when you are ready to begin. The timer will
              start immediately.
            </li>
          </ul>
          <button
